...
int main() {
    std::cout << "🚀 Testing Single Classifier Integration with Embedding System" << std::endl;
    ...
    try {
        ...
        if (!EmbeddingWithClassifier::initializeSingleClassifier(model_path, threshold, true)) {
            std::cerr << "❌ Failed to initialize single classifier. Exiting." << std::endl;
            return -1;
        }
        ...
        // Step 4: Test embedding and extraction with single classifier
        ...
        for (int i = 0; i < test_blocks_count; i++) {
            ...
            // Embed bit with automatic scheme selection (using single classifier)
            ...
            // Extract bit with automatic scheme prediction (using single classifier)
        ...
        if (!embedded_blocks.empty()) {
            ...
            std::string output_path = "result_with_single_classifier.png";
            if (writeImage(output_path, result_image)) {
                std::cout << "💾 Result saved to: " << output_path << std::endl;
            }
        }
        
        std::cout << "\n🎉 Single Classifier Test completed successfully!" << std::endl;
        std::cout << "🔬 Used final_model.pt for scheme selection" << std::endl;
