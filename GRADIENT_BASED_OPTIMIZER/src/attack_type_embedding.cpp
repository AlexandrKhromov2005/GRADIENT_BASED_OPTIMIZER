...
std::shared_ptr<AttackTypeClassifier> AttackTypeEmbedding::classifier_ = nullptr;
bool AttackTypeEmbedding::classifier_ready_ = false;

bool AttackTypeEmbedding::initializeAttackClassifier(const std::string& model_path, bool use_cuda) {
    std::cout << "🚀 Initializing AttackTypeEmbedding with model: " << model_path << std::endl;

    try {
        classifier_ = std::make_shared<AttackTypeClassifier>(model_path, use_cuda);
        classifier_ready_ = classifier_->isLoaded();

        if (classifier_ready_) {
            std::cout << "✅ AttackTypeEmbedding initialized successfully" << std::endl;
        } else {
            std::cerr << "❌ Failed to initialize AttackTypeEmbedding" << std::endl;
        }

        return classifier_ready_;
    } catch (const std::exception& e) {
        std::cerr << "❌ Exception during AttackTypeEmbedding initialization: " << e.what() << std::endl;
        classifier_ready_ = false;
        return false;
    }
}

cv::Mat AttackTypeEmbedding::embedWatermarkQuadrants(const cv::Mat& image_1024x1024,
...
cv::Mat AttackTypeEmbedding::extractWatermarkWithClassifier(const cv::Mat& image_1024x1024) {
    ...
    // Use classifier to predict attack type
    auto prediction = classifier_->predict(image_1024x1024, true);  // Use TTA

    // Convert to grayscale for watermark extraction
    cv::Mat gray_image;
    if (image_1024x1024.channels() == 3) {
        cv::cvtColor(image_1024x1024, gray_image, cv::COLOR_BGR2GRAY);
    } else {
    ...
    std::cout << "🎯 Attack Type Classifier prediction: " << prediction.class_name
              << " (confidence: " << (prediction.confidence * 100) << "%)" << std::endl;
    std::cout << "   Probabilities: NoAttack=" << prediction.prob_noattack
              << ", JPG70=" << prediction.prob_jpg70
              << ", JPG80=" << prediction.prob_jpg80
              << ", Contrast=" << prediction.prob_contrast << std::endl;

    // Map predicted attack to quadrant positions
    ...
    switch (prediction.predicted_class) {
        case 0:  // NoAttack -> NONE quadrants
            quadrant_positions = {{0,0}, {0,2}, {2,0}, {2,2}};
            attack_name = "NONE";
            break;
        case 1:  // JPG70 -> JPEG70 quadrants
            quadrant_positions = {{0,1}, {0,3}, {2,1}, {2,3}};
            attack_name = "JPEG70";
            break;
        case 2:  // JPG80 -> JPEG80 quadrants
            quadrant_positions = {{1,1}, {1,3}, {3,1}, {3,3}};
            attack_name = "JPEG80";
            break;
        case 3:  // ContrastIncrease -> CONTRAST quadrants
...
std::shared_ptr<AttackTypeClassifier> AttackTypeEmbedding::getClassifier() {
    return classifier_;
}
