...
- **Атака контрастом** - таблица на 256 значений, построенная тем же вызовом `convertTo`.
...
Алгоритм не упрощался: размер популяции, число итераций, целевая функция и каждое
...
поблочными потоками результаты статистически неотличимы (8 изображений x 10 seed, все
...
|---------------------------------------------------|----------|------------------|------------------|
| Базовый алгоритм, встраивание 512x512 (4096 бл.)  | 24.7 с   | 4.7 с            | **1.3 с** (x18)  |
| Квадрантный алгоритм, 1024x1024 (16384 блока)     | 158.8 с  | 22.3 с           | **6.2 с** (x26)  |
| Извлечение одного ЦВЗ (4096 блоков)               | 5.2 мс   | **0.5 мс** (x11) | -                |

В квадрантном конвейере время извлечения определяется классификатором типа атаки
(ResNet-50 на 1024x1024 с TTA-отражением, около 2 с на изображение на этом CPU); само
извлечение битов - те же 0.5 мс.
