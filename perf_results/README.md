...
| `seq_exact_base.txt`, `seq_exact_quad.txt` | Optimized single-thread code with the original single random stream (commit `147a03d`). Hashes, PSNR, SSIM and every BER are identical to the baseline files - only the time differs (24.7 s -> 4.7 s, 158.8 s -> 22.3 s). |
